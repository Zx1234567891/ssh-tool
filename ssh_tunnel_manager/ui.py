...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.host_tree.setToolTip("展开服务器可直接打开历史工作区；按住服务器并拖动可调整顺序")
    ...
    def _review_migrated_hosts(self) -> None:
        ...
        answer = QMessageBox.question(
            self, "旧版主机列表",
            f"检测到旧版保存的 {len(self.state.hosts)} 台主机。\n\n"
            "选择“是”保留现有列表；选择“否”清空应用列表并在需要时重新导入。\n"
            "无论如何都不会修改 SSH config，迁移前配置也已经备份。",
        )
