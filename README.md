...
- 为每台本地电脑和 SSH 主机创建独立 VSCode 配置；新终端自动使用该电脑选择的远程代理端口
...
4. 首次使用某台主机时，可点击“安装代理切换器”。软件打开的 SSH、VSCode 新终端和 Codex 会自动使用这台电脑配置的端口。

同一个远程用户可以从多台电脑同时登录。例如电脑 A 使用 `10099`、电脑 B 使用 `10098`；两台电脑从软件打开的 VSCode 新终端会分别保持自己的端口，不再修改共享的固定代理值。手动登录时仍可运行 `stm_proxy_use 10099` 或 `stm_proxy_off`。
